...
class Sensors:
    ...
    def getProximity(self): return self.apds9960.proximity

    def getColors(self): return self.apds9960.color_data

    def getTemperature(self): return self.bmp280.temperature

    def getStationPressure(self): return self.bmp280.pressure

    def getBaroPressure(self):
        temperature = self.getTemperature()
        station_pressure = self.getStationPressure()
        return station_pressure / math.exp(
                                   - kAltitude/((273.15+temperature)*29.263)
                                    )

    def getAltitude(self): return self.bmp280.altitude

    def getMagnetic(self): return np.array(self.lis3mdl.magnetic)

    def getAccel(self): return np.array(self.lsm6ds33.acceleration)

    def getGyro(self): return np.array(self.lsm6ds33.gyro)

    def getHumidity(self): return self.sht31d.relative_humidity
    ...
    def getEnvironmentalAsVector(self):
        ...
        return np.array((colors[0], colors[1], colors[2], colors[3],
                    self.getTemperature(), self.getBaroPressure(),
                    self.getHumidity(), self.getLoudness()))
    
    def toSensorPacket(self, board_id: int):
        return SensorPacket(board_id, self.getTemperature(), self.getBaroPressure(), self.getHumidity(), *self.getColors())